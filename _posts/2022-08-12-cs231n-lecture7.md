...
* 아래쪽 검은색 박스의 식으로부터 Nesterov는 다음과 같이 동작합니다.
    ...
    * $\tilde{x}_{t+1}$ 는 현재 위치 $\tilde{x}_{t}$ 와 velocity $v_{t+1}$ 를 더한 값에 (현재 velocity - 이전 velocity)에 일정 비율 $\rho$ 를 곱한 값을 더해주면 구하게 됩니다.

    * $\tilde{x}_{t+1}$ 는 마지막 식의 형태만 보면 됩니다.
        * 현재 위치 $\tilde{x}_{t}$ 와 velocity $v_{t+1}$ 를 더한 값에 (현재 velocity - 이전 velocity)에 일정 비율 $\rho$ 를 곱한 값을 더해주면 구하게 됩니다.

        * 현재 위치 $\tilde{x}_{t}$ 와 velocity $v_{t+1}$ 를 더한 값에 (현재 velocity $v_{t+1}$ - 이전 velocity $v_t$ )에 일정 비율 $\rho$ 를 곱한 값을 더해주며 구하게 됩니다.
