...
그 중에서 가장 많이 쓰이는 L1 regularization과 L2 regularization의 특징은 다음과 같습니다.

* L1 regularization
    * L1 norm으로 W에 패널티를 부과하는 것
    * 통상적으로 값을 빼줌
    * 작은 가중치는 0으로 수렴 -> 중요한 가중치만 남게 됨
    * 즉, 의미 있는 값을 원하면 L1 regularization이 좋음
* L2 regularization(Weight decay)
    * 가중치 행렬 W의 L2 norm으로 패널티를 주는 것
    * 가중치를 0에 가깝게 유도
    * 데이터를 spread 하게 바라봄 -> 모든 데이터를 고려함
    * 일반적으로 많이 사용
...
* L2 regularization
    * W에서 특정 값만 모델에 큰 영향을 미치도록 하지 않습니다.
    * W의 값이 고르고 넓게 퍼지도록 합니다.
