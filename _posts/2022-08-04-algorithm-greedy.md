...
* 이 문제를 풀려면 가장 먼저 반복되는 수열에 대해서 파악해야 합니다.
...
* 이 문제를 푸는 아이디어는 바로 각 행마다 가장 작은 수를 찾은 뒤에 그 수중에서 가장 큰 수를 찾는 것입니다.
...
* 해당 문제는 주어진 N에 대하여 '최대한 많이 나누기'를 수행하면 됩니다.
    * 왜냐하면 어떠한 수가 있을 때, '2 이상의 수로 나누는 것'이 '1을 빼는 것'보다 숫자를 훨씬 많이 줄일 수 있기 때문입니다. *  문제에서는 K가 2 이상의 자연수이므로, 가능하면 나누는 것이 항상 더 숫자를 빠르게 줄이는 방법이 됩니다.
